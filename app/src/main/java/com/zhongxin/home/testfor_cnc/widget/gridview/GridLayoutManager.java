...
final class GridLayoutManager extends RecyclerView.LayoutManager
{
	...
	private Grid.Provider mGridProvider = new Grid.Provider()
	{
		...
		public int createItem(int index, boolean append, Object[] item)
		{
			...
			if (!lp.isItemRemoved())
			{
				...
				if (!mInLayout)
				{
					...
					if (index == mFocusPosition
					        ...
					        && (mPendingMoveSmoothScroller == null || mPendingMoveSmoothScroller.mPendingMoves == 0))
					{
			...
			item[0] = v;
			return mOrientation == HORIZONTAL ? v.getMeasuredWidth() : v
			        .getMeasuredHeight();
		}
		        ...
		        int edge)
		{
			...
			{
				edge = !mGrid.isReversedFlow() ? mWindowAlignment.mainAxis()
	...
	{
		mWindowAlignment.mainAxis().setWindowAlignmentOffsetPercent(
		        offsetPercent);
	}
	...
	private int getViewPrimarySize(View view)
	{
		...
		return mOrientation == HORIZONTAL ? p.getOpticalWidth(view) : p
		        .getOpticalHeight(view);
	}
	...
	{
		// 如果是翻页效果就不释放
		if (mBaseGridView.getFocusScrollStrategy() != BaseGridView.FOCUS_SCROLL_PAGE)
		{
			mRecycler = null;
			mState = null;
		}
	...
	private boolean layoutInit()
	{
		...
		else
		{
			...
			int firstVisibleIndex = focusViewWasInTree ? mGrid
	...
	private boolean processRowSizeSecondary(boolean measure)
	{
		...
		CircularIntArray[] rows = mGrid == null ? null : mGrid
		        .getItemPositionsInRows();
		...
		for (int rowIndex = 0; rowIndex < mNumRows; rowIndex++)
		{
			...
			for (int rowItemPairIndex = 0; rowItemPairIndex < rowItemsPairCount; rowItemPairIndex += 2)
			{
				...
				for (int i = rowIndexStart; i <= rowIndexEnd; i++)
				{
					...
					final int secondarySize = mOrientation == HORIZONTAL ? view
			...
			if (measure && rowSize < 0 && itemCount > 0)
			{
				if (scrapChildWidth < 0 && scrapChildHeight < 0)
				{
					...
					measureScrapChild(position, MeasureSpec.makeMeasureSpec(0,
		...
		if (mRowSizeSecondaryRequested == ViewGroup.LayoutParams.WRAP_CONTENT)
		{
			...
			switch (modeSecondary)
			{
				...
				case MeasureSpec.AT_MOST:
					measuredSizeSecondary = Math.min(getSizeSecondary()
                            + paddingSecondary, mMaxSizeSecondary);
					break;
			...
			switch (modeSecondary)
			{
				...
				case MeasureSpec.EXACTLY:
					...
					else if (mRowSizeSecondaryRequested == 0)
					{
						...
						mFixedRowSizeSecondary = (sizeSecondary
						        - paddingSecondary - mMarginSecondary
						        * (mNumRows - 1))
						        / mNumRows;
					}
					else
	...
	private void measureChild(View child)
	{
		...
		final int secondarySpec = (mRowSizeSecondaryRequested == ViewGroup.LayoutParams.WRAP_CONTENT) ? MeasureSpec
		        .makeMeasureSpec(0, MeasureSpec.UNSPECIFIED) : MeasureSpec
		        .makeMeasureSpec(mFixedRowSizeSecondary, MeasureSpec.EXACTLY);
		...
		if (DEBUG)
			Log.v(getTag(), "child lp width " + lp.width + " height "
                    + lp.height);
		if (TRACE)
			TraceHelper.endSection();
	}
	        ...
	        Class<? extends E> facetClass)
	{
		...
		{
			FacetProvider p = mFacetProviderAdapter.getFacetProvider(vh
			        .getItemViewType());
			if (p != null)
			{
	        ...
	        int startSecondary)
	{
		...
		final int horizontalGravity = (mReverseFlowPrimary || mReverseFlowSecondary) ? Gravity
		        .getAbsoluteGravity(mGravity
                                & Gravity.RELATIVE_HORIZONTAL_GRAVITY_MASK,
                        View.LAYOUT_DIRECTION_RTL) : mGravity
	...
	{
		LayoutParams p = (LayoutParams) v.getLayoutParams();
		p.setOpticalInsets(left - v.getLeft(), top - v.getTop(), v.getRight()
		        - right, v.getBottom() - bottom);
	}
	...
	private void fastRelayout()
	{
		...
		for (int index = 0; index < childCount; index++)
		{
			...
			{
				if (DEBUG)
					Log.w(getTag(), "fastRelayout(): no Location at "
                            + position);
			...
			if (oldPrimarySize != primarySize)
			{
				...
				if (DEBUG)
					Log.d(getTag(), "fastRelayout: view size changed at "
                            + position);
				invalidateAfter = true;
				break;
			}
	...
	{
		int highVisiblePos = (!mReverseFlowPrimary) ? mGrid
		        .getLastVisibleIndex() : mGrid.getFirstVisibleIndex();
	...
	{
		int lowVisiblePos = (!mReverseFlowPrimary) ? mGrid
		        .getFirstVisibleIndex() : mGrid.getLastVisibleIndex();
	...
	private void updateScrollController()
	{
		...
		if (DEBUG)
		{
			Log.v(getTag(), "updateScrollController mSizePrimary "
                    + mSizePrimary + " mWindowAlignment " + mWindowAlignment
                    + " mScrollOffsetPrimary " + mScrollOffsetPrimary);
		}
	}
	...
	public boolean requestChildRectangleOnScreen(RecyclerView parent,
	        View view, Rect rect, boolean immediate)
	{
		if (DEBUG)
			Log.v(getTag(), "requestChildRectangleOnScreen " + view + " "
                    + rect);
		return false;
	}
	...
	private int getPrimarySystemScrollPosition(View view)
	{
		...
		if (!mReverseFlowPrimary)
		{
			...
			isMax = mGrid.getLastVisibleIndex() == (mState == null ? getItemCount()
			        : mState.getItemCount()) - 1;
		}
		else
		{
			isMax = mGrid.getFirstVisibleIndex() == 0;
			isMin = mGrid.getLastVisibleIndex() == (mState == null ? getItemCount()
			        : mState.getItemCount()) - 1;
		}
		...
		return mWindowAlignment.mainAxis().getSystemScrollPos(
		        viewCenterPrimary, isMin, isMax);
	}
	...
	private int getSecondarySystemScrollPosition(View view)
	{
		...
		return mWindowAlignment.secondAxis().getSystemScrollPos(
		        viewCenterSecondary, isMin, isMax);
	}
		...
		else if (viewMax > clientSize + paddingLow)
		{
			...
			if (mFocusScrollStrategy == BaseGridView.FOCUS_SCROLL_PAGE)
			{
				...
				do
				{
					...
					lastView = findViewByPosition(positions.get(positions
					        .size() - 1));
	...
	private boolean getAlignedPosition(View view, View childView, int[] deltas)
	{
		...
		if (DEBUG)
		{
			Log.v(getTag(), "getAlignedPosition " + scrollPrimary + " "
	        ...
	        ArrayList<View> views, int direction, int focusableMode)
	{
		...
		if (recyclerView.hasFocus())
		{
			...
			final int focusedRow = mGrid != null && focusedPos != NO_POSITION ? mGrid
			        .getLocation(focusedPos).row : NO_POSITION;
	...
	{
		if (DEBUG)
			Log.v(getTag(), "onSaveInstanceState getSelection() "
                    + getSelection());
	...
	public void onRestoreInstanceState(Parcelable state)
	{
		...
		requestLayout();
		if (DEBUG)
			Log.v(getTag(), "onRestoreInstanceState mFocusPosition "
                    + mFocusPosition);
	}
	...
	private int computeScrollOffset(State state)
	{
		...
		else
		{
			...
			return ScrollbarHelper.computeScrollOffset(getItemCount(),
			        this.mOrientationHelper, this
			                .findFirstVisibleChildClosestToStart(
			                        !this.mSmoothScrollbarEnabled, true), this
	...
	final static class SavedState implements Parcelable
	{
		...
		{
			index = in.readInt();
			childStates = in.readBundle(GridLayoutManager.class
			        .getClassLoader());
		}
	...
	final class PendingMoveSmoothScroller extends GridLinearSmoothScroller
	{
		...
		void consumePendingMovesBeforeLayout()
		{
			...
			for (int pos = startPos; mPendingMoves != 0; pos = mPendingMoves > 0 ? pos
			        + mNumRows
			        : pos - mNumRows)
			{
		...
		void consumePendingMovesAfterLayout()
		{
			if (mStaggeredGrid && mPendingMoves != 0)
			{
				...
				final int focusedRow = mGrid != null
				        && mFocusPosition != NO_POSITION ? mGrid
				        .getLocation(mFocusPosition).row : NO_POSITION;
				View newSelected = null;
